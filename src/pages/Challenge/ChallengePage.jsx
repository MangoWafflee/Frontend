...
export default function ChallengePage() {
  ...
  const participateChallenge = async () => {
    const response = await axios.post("/userchallenge/participate", {
      uid: user.uid,
    });
    return response;
  };
  ...
  const fetchUserChallenges = async () => {
    ...
    const categorizedChallenges = {
      ...
      participatingChallengeList: data.filter(
        (challenge) =>
          challenge.participating === true && challenge.success_status === null
      ),
      // 참여했던 챌린지 데이터
  ...
  const getOngoingChallenges = async (userChallenges) => {
    ...
    const ongoingChallenges = data.filter((challenge) => {
      ...
      return !userChallenges.participatingChallengeList.some(
        (participatingChallenge) => participatingChallenge.id === challenge.id
      );
    });
  ...
  if (userChallengesError || ongoingChallengesError) {
    return (
      <div>
        유저 관련 챌린지 정보 오류 {userChallengesError} <br /> 진행중인 챌린지 정보 오류 {ongoingChallengesError}
      </div>
    );
  }
  ...
  return (
    <div className="challenge-page">
        ...
        userChallenges.participatingChallengeList.map((challenge) => (
          ...
          >
            ...
            <div className="column">
              ...
              <p>{`${calculateDateDifference(challenge.endDate)}일 남음`}</p>
            </div>
      ...
      <Divider />
      <h2>챌린지 참여하기</h2>
      {ongoingChallenges.onGoingChallengeList.length === 0 ||
      ongoingChallenges.onGoingChallengeList === null ? (
        <div className="challenge">진행중인 챌린지가 없습니다.</div>
      ) : (
        ongoingChallenges.onGoingChallengeList.map((challenge) => (
          <div
            className="challenge"
            onClick={() => {
              setSelectedChallenge(challenge);
      ...
      >
        {selectedChallenge && (
          <div>
            ...
            <br />
            <p>
              챌린지에 참가해볼 시간입니다! 이번 달에 ~를 하고 챌린지를 성공해보세요!
            </p>
            <br />
